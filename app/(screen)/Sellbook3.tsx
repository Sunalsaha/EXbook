...
const SellBook3: React.FC = () => {
  ...
  const [location, setLocation] = useState(
    'Action Area I 1/2, Newtown, New Town, Cha  DG Black(Newtown) uttar 24 pargana West Bengal 74.....'
  );
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets && result.assets[0]) {
      ...
      if (photos.length < 9) {
        Alert.alert(
          'Photo Added',
          `${photos.length + 1} / 10 photos. Add another?`,
          [
            { text: 'Done', style: 'cancel' },
            { text: 'Add More', onPress: pickImage },
          ]
        );
      }
    }
